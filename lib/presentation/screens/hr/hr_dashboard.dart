...
class _HRDashboardState extends State<HRDashboard> {
  ...
  Widget _buildTeamOverview() {
    return Consumer2<LeaveProvider, TimeLogProvider>(
      builder: (context, leaveProvider, timeLogProvider, _) {
        ...
        final todayLeaves = allLeaves.where((l) {
        ...
        final pendingApprovals = allLeaves
            .where((l) => l.status == 'pending')
            .length;
        final activeToday = timeLogProvider.allTimeLogs
  ...
  Widget _buildApprovalCard(leave) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
